...
export default function StockInstitutionalModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-2 sm:p-4 md:p-6 backdrop-blur-md animate-in fade-in duration-200">
      <div className="relative flex max-h-[94vh] w-full max-w-7xl flex-col overflow-hidden rounded-2xl border border-slate-700/80 bg-[#081225] text-slate-100 shadow-2xl">
        {/* MODAL HEADER */}
        <div className="flex flex-wrap items-center justify-between border-b border-slate-800 bg-[#080E1A]/90 px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2.5">
                ...
                <span className="rounded-md border border-slate-700 bg-slate-800/80 px-2 py-0.5 text-xs font-semibold text-cyan-400">
                  NSE: {symbol}
                </span>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={onClose}
              className="flex h-9 w-9 items-center justify-center rounded-xl border border-slate-700 bg-slate-800/80 text-slate-400 hover:border-slate-600 hover:bg-slate-700 hover:text-white transition"
            >
              <X size={18} />
            </button>
          </div>
        ...
        {loading ? (
          <div className="flex h-96 flex-col items-center justify-center gap-3">
            ...
            <p className="text-sm text-slate-400">Synthesizing institutional filings for {symbol}...</p>
          </div>
        ) : error ? (
          <div className="flex h-96 flex-col items-center justify-center gap-2 text-rose-400">
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 custom-scrollbar">
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-6">
              <div className="rounded-xl border border-slate-800 bg-[#080E1A]/80 p-3">
                <span className="text-[11px] font-medium text-slate-400">Smart Money Score</span>
              ...
              <div className="rounded-xl border border-slate-800 bg-[#080E1A]/80 p-3">
                <span className="text-[11px] font-medium text-slate-400">Total MF Equity Stake</span>
                <div className="mt-1 flex items-baseline gap-2">
              ...
              <div className="rounded-xl border border-slate-800 bg-[#080E1A]/80 p-3">
                <span className="text-[11px] font-medium text-slate-400">Free Float Absorbed</span>
                <div className="mt-1 flex items-baseline gap-2">
              ...
              <div className="rounded-xl border border-slate-800 bg-[#080E1A]/80 p-3">
                <span className="text-[11px] font-medium text-slate-400">Active Alpha Schemes</span>
                <div className="mt-1 flex items-baseline gap-2">
              ...
              <div className="rounded-xl border border-slate-800 bg-[#080E1A]/80 p-3">
                <span className="text-[11px] font-medium text-slate-400">Net MoM Inflow</span>
                <div className="mt-1 flex items-baseline gap-2">
              ...
              <div className="rounded-xl border border-slate-800 bg-[#080E1A]/80 p-3">
                <span className="text-[11px] font-medium text-slate-400">Stealth Accumulation</span>
                <div className="mt-1">
                  {data.latest_stats.is_stealth_accumulation ? (
                    ...
                    <span className="text-xs text-slate-500">Normal Range</span>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
              ...
              <div className="lg:col-span-7 flex flex-col rounded-xl border border-slate-800 bg-[#080E1A]/80 p-4 sm:p-5">
                <div className="flex flex-wrap items-center justify-between gap-2 mb-4 pb-3 border-b border-slate-800">
                  ...
                  {/* FILTER BUTTONS */}
                  <div className="flex items-center gap-1 rounded-lg border border-slate-800 bg-slate-900/90 p-1 text-[11px]">
                    <button
                      onClick={() => setFilterCategory("ALL")}
                      className={`rounded-md px-2.5 py-1 font-semibold transition ${
                        filterCategory === "ALL" ? "bg-cyan-500/20 text-cyan-400" : "text-slate-400 hover:text-white"
                      }`}
                    >
                    ...
                    <button
                      ...
                      className={`rounded-md px-2.5 py-1 font-semibold transition ${
                        filterCategory === "ACTIVE" ? "bg-cyan-500/20 text-cyan-400" : "text-slate-400 hover:text-white"
                      }`}
                    >
                      Active Alpha
                    </button>
                    <button
                      ...
                      className={`rounded-md px-2.5 py-1 font-semibold transition ${
                        filterCategory === "BUYERS" ? "bg-emerald-500/20 text-emerald-400" : "text-slate-400 hover:text-white"
                      }`}
                    >
                      Buyers Only
                    </button>
                        ...
                        return (
                          <tr key={i} className="hover:bg-slate-800/40 transition">
                            <td className="py-2.5 pr-2">
                              <div className="font-semibold text-white">{h.scheme_name}</div>
                            ...
                            <td className="py-2.5 text-right font-mono text-cyan-400 font-semibold">
                              ₹{h.market_value_cr}
                            </td>
                            ...
                            <td className="py-2.5 text-right">
                              {h.holding_status === "NEW_ENTRY" ? (
                                <span className="inline-flex items-center gap-0.5 rounded-md border border-cyan-500/40 bg-cyan-500/10 px-2 py-0.5 text-[10px] font-bold text-cyan-400">
                                  <Sparkles size={10} /> NEW ENTRY
                                </span>
                              ) : isAdd ? (
                                <span className="inline-flex items-center gap-0.5 rounded-md border border-emerald-500/40 bg-emerald-500/10 px-2 py-0.5 text-[10px] font-bold text-emerald-400">
                                  <ArrowUpRight size={12} /> +{h.mom_shares_change_pct}%
                                </span>
                              ) : isTrim ? (
                                <span className="inline-flex items-center gap-0.5 rounded-md border border-rose-500/40 bg-rose-500/10 px-2 py-0.5 text-[10px] font-bold text-rose-400">
                                  <ArrowDownRight size={12} /> {h.mom_shares_change_pct}%
                                </span>
                              ) : (
                                <span className="text-[10px] font-medium text-slate-400">UNF</span>
                              )}
                            </td>
                          </tr>
                        );
              ...
              <div className="lg:col-span-5 flex flex-col gap-4">
                ...
                <div className="rounded-xl border border-slate-800 bg-[#080E1A]/80 p-4">
                  <div className="flex items-center justify-between mb-2">
                    <h4 className="text-xs font-bold text-slate-200 flex items-center gap-1.5">
                      <BarChart3 size={14} className="text-cyan-400" />
                      Institutional Ownership Timeline
                    </h4>
                    <span className="text-[10px] text-slate-400">6-Month Trend</span>
                  </div>

                  {/* MINI SVG LINE CHART */}
                  <div className="h-32 w-full pt-2">
                    <div className="flex h-20 items-end gap-2 border-b border-slate-800 px-2">
                    ...
                    <div className="flex justify-between px-1 pt-1 text-[10px] text-slate-400">
                ...
                <div className="rounded-xl border border-slate-800 bg-[#080E1A]/80 p-4">
                  ...
                  <div className="rounded-lg border border-cyan-500/20 bg-cyan-500/5 p-2.5 mb-2.5">
                ...
                <div className="rounded-xl border border-emerald-500/40 bg-[#080E1A]/90 p-4 shadow-lg">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="rounded-md bg-emerald-500/20 px-2 py-0.5 text-xs font-black text-emerald-400">
                      {data.action_zone.action}
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 text-center">
                    <div className="rounded-lg border border-slate-800 bg-slate-900/80 p-2">
                      <span className="text-[10px] text-slate-400 block">Entry Base Zone</span>
                    ...
                    <div className="rounded-lg border border-slate-800 bg-slate-900/80 p-2">
                      <span className="text-[10px] text-slate-400 block">Base Stop Loss</span>
